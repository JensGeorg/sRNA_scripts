...
#call:
#R --slave -f /home/jens/CopraRNA-git/coprarna_aux/conserved_site_on_subset.r --args sel=866,1494,1472,1466,4617,1465,3449,3752,2057,2490,2967,1075,3358,14360,6799,4208,5037,3046,4682,14540
...
d<-dir()
existing_results<-grep("gene_out.Rdata",d)
...
int_sites2<-int_sites
peak_list2<-peak_list

# get absolute path
initial.options <- commandArgs(trailingOnly = FALSE)
path<-initial.options [4]
path<-sub("conserved_site_on_subset.r","",path)
#print(path)
...
max_cores<-1 # rgl plot interferes with multicore processing
...
rgl_plot<-function(align_table_int_pos=align_table_int_pos, test=test, tab_aligned=tab_aligned,tabsub_aligned=tabsub_aligned,alignment=alignment,sRNA_alignment2=sRNA_alignment2,na3=na3,peaks1=peaks1,tab=tab){
	...
	rgl.snapshot( filename=paste(na3,"/",tab[1,"name"],"_interaction_porb_landscape.png",sep=""), fmt = "png", top = TRUE )
	rgl.close()
...
dat<-cbind(dat,sel)
	...
	ji<-1
			...
			if(length(temp)>1){
				...
				if(length(test[[3]])>0){
					...
					if(length(peaks_all)>0){
						...
						colo<-c("#799943","#5680b0","#9c6488","#e4c054","#e26a5f","#b43768","#006362","#614736","#294258","#77b1b5")
						colo3<-c()
						for(jj in 1:length(peaks1)){
							fc <- colorRampPalette(c(colo[jj], colo2[jj]))
							tmp<-fc(length(peaks1[[jj]]))
							names(tmp)<-peaks1[[jj]]
							colo3<-c(colo3,tmp)
						}
						colo3<-c(colo3,"#b0b0b0")
		...
		names(int_sites)<-dat[,"sel"]
		names(peak_list)<-dat[,"sel"]
		names(col_list)<-dat[,"sel"]




	dat<-dat_old
	if(exists("col_list2")==F){
		col_list2<-vector("list", nrow(dat))
	}
	tmp<-as.numeric(names(int_sites))
	
	int_sites2[tmp]<-int_sites
	peak_list2[tmp]<-peak_list
	tmp<-as.numeric(names(col_list))
	col_list2[tmp]<-col_list


gene_out<-list(int_sites2,peak_list2,col_list2)	
	
save(gene_out, file="gene_out.Rdata")	


int_sites<-int_sites2
